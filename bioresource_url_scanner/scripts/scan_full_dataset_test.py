#!/usr/bin/env python3
"""
scan_full_dataset_test.py - TEST MODE: Scan 20 random URLs to verify scanner works

Run this first to test before running the full scan.
"""
...
class BioresourceScanner:
    def __init__(self, max_workers=10, domain_delay=1.0):
        ...
        self.session.headers.update({
            'User-Agent': 'BioresourceScanner/3.0 TEST (Biodata Inventory Project)'
        })
...
output_path = "data/test_scan_results.csv"
...
for idx, (_, row) in enumerate(top_5.iterrows(), 1):
    ...
    print(f"   {idx}. {row['total_score']:2.0f} pts | {status} | {entity}")

print(f"\n📁 Saved to: {output_path}")
print(f"\n{'=' * 80}")
print(f"✅ TEST PASSED - Ready to run full scan!")
print(f"   Run: python scripts/scan_full_dataset.py")
print(f"{'=' * 80}")
