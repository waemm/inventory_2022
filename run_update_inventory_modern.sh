...
# Compatible with Python 3.11 and updated packages
# Date: 2025-10-21

set -e  # Exit on any error

# Use modern Python environment
export PYTHON_CMD="biodata_modern_env/bin/python"
export PIP_CMD="biodata_modern_env/bin/pip"
export PYTHONPATH="src:$PYTHONPATH"

# Set variables from config file
export project_env="./biodata_modern_env"
...
TEST_MODE=false    # New option for testing with limited data
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --test-mode                       Use small subset for URL checking (faster testing)"
...
echo "========================================="
echo "Modern Python Environment (Python 3.11)"
...
echo "Test mode (limited URL checking): $TEST_MODE"
echo ""

# Activate modern environment
echo "Activating modern environment..."
...
if ! $PYTHON_CMD -c "import torch, transformers, numpy; print(f'✅ Libraries loaded: PyTorch {torch.__version__}, Transformers {transformers.__version__}, NumPy {numpy.__version__}')" 2>/dev/null; then
    echo "❌ Environment verification failed. Please check your modern environment setup."
    echo "Run: source activate_modern.sh"
    echo "See MODERN_ENVIRONMENT_SETUP.md for setup instructions."
    exit 1
fi

# Verify compatible package versions
echo "Verifying compatible package versions..."
$PYTHON_CMD -c "
...
for pkg, ver in expected.items():
    if versions[pkg] != ver:
        warnings.append(f'⚠️ {pkg}: expected {ver}, found {versions[pkg]}')
if warnings:
    print('Package version warnings:')
    for w in warnings:
        print(w)
    print('Pipeline may still work but compatibility is not guaranteed.')
else:
...
if [ "$jump_to_manual_continue" = false ]; then
    ...
    echo "Model: out/classif_train_out/article_classifier.pt"
...
"
    ...
    echo "Model: out/ner_train_out/named_entity_recognition.pt"
...
if [ "$SKIP_MANUAL_REVIEW" = true ]; then
    ...
    input_file="$for_manual_review_dir/predictions.csv"
    ...
    jump_to_url_check=true
    input_file="$manually_reviewed_dir/predictions.csv"
else
    echo "========================================="
    ...
    echo "./run_update_inventory_modern.sh --continue-after-manual-review"
    echo ""
    echo "Or to skip manual review entirely, run:"
    echo "./run_update_inventory_modern.sh --skip-manual-review"
    exit 0
fi

if [ "$jump_to_url_check" = true ]; then
    # Prepare input file for URL checking (with test mode option)
    if [ "$TEST_MODE" = true ]; then
        echo "Test mode: Creating subset of entries for faster processing..."
        $PYTHON_CMD -c "
import pandas as pd
df = pd.read_csv('$input_file')
test_df = df.head(10)
test_df.to_csv('$for_manual_review_dir/predictions_test.csv', index=False)
print(f'Created test file with {len(test_df)} entries out of {len(df)} total')
"
        url_input_file="$for_manual_review_dir/predictions_test.csv"
        url_output_suffix="_test"
        echo "Using test file for URL checking: $url_input_file"
    else
        url_input_file="$input_file"
        url_output_suffix=""
        echo "Using full file for URL checking: $url_input_file"
    fi
    ...
    url_checked=$(tail -n +2 "$check_url_dir/predictions${url_output_suffix}.csv" | wc -l)
    ...
    echo "========================================="
    echo "Input: $check_url_dir/predictions${url_output_suffix}.csv"
    echo "Chunk size: $epmc_chunk_size"
    echo "Output: $epmc_meta_dir/predictions${url_output_suffix}.csv"
    echo ""

    $PYTHON_CMD src/get_meta.py \
        --file "$check_url_dir/predictions${url_output_suffix}.csv" \
        -s "$epmc_chunk_size" \
        -o "$epmc_meta_dir"

    echo "✅ EuropePMC metadata retrieval completed successfully"
    meta_entries=$(tail -n +2 "$epmc_meta_dir/predictions${url_output_suffix}.csv" | wc -l)
    ...
    echo "========================================="
    echo "Input: $epmc_meta_dir/predictions${url_output_suffix}.csv"
    echo "Country format: $country_format"
    echo "Output: $processed_countries/predictions${url_output_suffix}.csv"
    echo ""

    $PYTHON_CMD src/process_countries.py \
        -o "$processed_countries" \
        -f "$country_format" \
        "$epmc_meta_dir/predictions${url_output_suffix}.csv"

    echo "✅ Country processing completed successfully"
    final_entries=$(tail -n +2 "$processed_countries/predictions${url_output_suffix}.csv" | wc -l)
    ...
    echo "========================================="
    echo "Final inventory file: $processed_countries/predictions${url_output_suffix}.csv"
    echo ""
    ...
    echo "- Final inventory entries: $final_entries"
    echo ""
    echo "Modern environment features:"
    echo "✅ Python 3.11 compatibility"
    echo "✅ Updated package versions with compatibility fixes"
    echo "✅ Enhanced error handling and logging"
    echo "✅ Test mode for faster development iterations"
fi
...
if [ "$jump_to_url_check" = true ]; then
echo "✅ URL Checking: $check_url_dir/predictions${url_output_suffix}.csv"
echo "✅ EuropePMC Metadata: $epmc_meta_dir/predictions${url_output_suffix}.csv"
echo "✅ Final Inventory: $processed_countries/predictions${url_output_suffix}.csv"
fi
echo ""
echo "For full documentation, see: pipeline_modern_steps.md"
echo "For environment setup, see: MODERN_ENVIRONMENT_SETUP.md"
echo ""